...
for q in range(rep):
    beta[q][0] = st.multivariate_normal.rvs(np.array([0, 0, 0, 0]), np.array([[5, 0, 0, 0], [0, 5, 0, 0],
                                                                              [0, 0, 5, 0], [0, 0, 0, 5]]), size=1)
    ...
    sigma_beta[q][0] = np.array([[5, 0, 0, 0], [0, 5, 0, 0], [0, 0, 5, 0], [0, 0, 0, 5]])
    ...
    for z in range(1, s):
        ...
        la1 = min(1.0, (f(y[q], ab1, p1[q][z - 1], p2[q][z - 1], sigma[q][z - 1]) /
                  ...
                  (st.multivariate_normal.pdf(ab1, np.array([0, 0, 0, 0]),
                                              np.array([[5, 0, 0, 0], [0, 5, 0, 0], [0, 0, 5, 0], [0, 0, 0, 5]])) /
...
def f(y1, beta1, sigma1):
    ...
    for k in range(n - 3):
        if y1[k + 3] <= (beta1[0] + y1[k + 2] * beta1[1] + y1[k + 1] * beta1[2] + y1[k] * beta1[3]):
            q1[k] = np.exp(-(beta1[0] + y1[k + 2] * beta1[1] + beta1[2] * y1[k + 1] + beta1[3] * y1[k] - y1[
                k + 3]) * 2 / sigma1) / sigma1
        else:
            q1[k] = np.exp(-(y1[k + 3] - beta1[0] - beta1[1] * y1[k + 2] - beta1[2] * y1[k + 1] - beta1[3] * y1[
                k]) * 2 / sigma1) / sigma1
    return np.prod(q1)
...
for q in range(rep):
    ...
    for z in range(1, s):
        ...
        la1 = min(1.0, (f(y[q], ab1, sigma[q][z - 1]) / f(y[q], beta[q][z - 1], sigma[q][z - 1])) *
                  (st.multivariate_normal.pdf(ab1, np.array([0, 0, 0, 0]), np.array([[5, 0, 0, 0], [0, 5, 0, 0],
                                                                                     [0, 0, 5, 0], [0, 0, 0, 5]])) /
                   st.multivariate_normal.pdf(beta[q][z - 1], np.array([0, 0, 0, 0]),
                                              np.array([[5, 0, 0, 0], [0, 5, 0, 0], [0, 0, 5, 0], [0, 0, 0, 5]]))) *
                  (st.multivariate_normal.pdf(beta[q][z - 1], mean=mu_beta[q][z - 1], cov=sigma_beta[q][z - 1]) /
    ...
    def g(y1, k1, beta1, sigma1):
        if y1[k1 + 3] <= (beta1[0] + y1[k1 + 2] * beta1[1] + y1[k1 + 1] * beta1[2] + y1[k1] * beta1[3]):
            q1 = np.exp(-(beta1[0] + y1[k1 + 2] * beta1[1] + beta1[2] * y1[k1 + 1] + beta1[3] * y1[k1] - y1[
                k1 + 3]) * 2 / sigma1) / sigma1
        else:
            q1 = np.exp(-(y1[k1 + 3] - beta1[0] - beta1[1] * y1[k1 + 2] - beta1[2] * y1[k1 + 1] - beta1[3] * y1[
                k1]) * 2 / sigma1) / sigma1
        return q1
