...
x = pd.read_csv('data2.csv', encoding='utf-8', header=None)
x.columns = ['a', 'b']
...
def f(y1, beta1, p11, p21, sigma1):
    ...
    for k in range(n - 4):
        if y1[k + 4] <= (
                beta1[0] + y1[k + 3] * beta1[1] + y1[k + 2] * beta1[2] + y1[k + 1] * beta1[3] + y1[k] * beta1[4]):
            q1[k] = np.exp(
                -np.power((beta1[0] + y1[k + 3] * beta1[1] + y1[k + 2] * beta1[2] + y1[k + 1] * beta1[3] + y1[k] *
                           beta1[4] - y1[k + 4]) * (2 * ss.gamma(1 + 1 / p11)) / sigma1, p11)) / sigma1
        else:
            q1[k] = np.exp(
                -np.power((y1[k + 4] - beta1[0] - y1[k + 3] * beta1[1] - y1[k + 2] * beta1[2] - y1[k + 1] * beta1[3] -
                           y1[k] * beta1[4]) * (2 * ss.gamma(1 + 1 / p21)) / sigma1, p21)) / sigma1
    return np.prod(q1)


beta_f = np.zeros(shape=(rep, s - s1, 5))
...
mu_beta = np.zeros(shape=(rep, s, 5))
sigma_beta = np.zeros(shape=(rep, s, 5, 5))
...
for q in range(rep):
    beta[q][0] = st.multivariate_normal.rvs(np.array([0, 0, 0, 0, 0]), np.array([[5, 0, 0, 0, 0], [0, 5, 0, 0, 0],
                                                                                 [0, 0, 5, 0, 0], [0, 0, 0, 5, 0],
                                                                                 [0, 0, 0, 0, 5]]), size=1)
    ...
    mu_beta[q][0] = np.array([0, 0, 0, 0, 0])
    sigma_beta[q][0] = np.array([[5, 0, 0, 0, 0], [0, 5, 0, 0, 0], [0, 0, 5, 0, 0], [0, 0, 0, 5, 0], [0, 0, 0, 0, 5]])
    ...
    for z in range(1, s):
        ...
        la1 = min(1.0, (f(y[q], ab1, p1[q][z - 1], p2[q][z - 1], sigma[q][z - 1]) /
                  ...
                  (st.multivariate_normal.pdf(ab1, np.array([0, 0, 0, 0, 0]), np.array(
                      [[5, 0, 0, 0, 0], [0, 5, 0, 0, 0], [0, 0, 5, 0, 0], [0, 0, 0, 5, 0], [0, 0, 0, 0, 5]])) /
                   st.multivariate_normal.pdf(beta[q][z - 1], np.array([0, 0, 0, 0, 0]),
                                              np.array(
    ...
    def g(y1, k1, beta1, p11, p21, sigma1):
        if y1[k1 + 4] <= (
                beta1[0] + y1[k1 + 3] * beta1[1] + y1[k1 + 2] * beta1[2] + y1[k1 + 1] * beta1[3] + y1[k1] * beta1[4]):
            q1 = np.exp(
                -np.power((beta1[0] + y1[k1 + 3] * beta1[1] + y1[k1 + 2] * beta1[2] + y1[k1 + 1] * beta1[3] +
                           y1[k1] * beta1[4] - y1[k1 + 4]) * (2 * ss.gamma(1 + 1 / p11)) / sigma1, p11)) / sigma1
        else:
            q1 = np.exp(
                -np.power(
                    (y1[k1 + 4] - beta1[0] - y1[k1 + 3] * beta1[1] - y1[k1 + 2] * beta1[2] - y1[k1 + 1] * beta1[3] -
                     y1[k1] * beta1[4]) * (2 * ss.gamma(1 + 1 / p21)) / sigma1, p21)) / sigma1
        return q1
...
fi2 = np.zeros(shape=(rep, s - s1))
for r in range(rep):
    fi2[r] = beta[:, s1:s][r][:, 2]
fi_2 = fi2.reshape((s - s1) * rep, order='F')

fi3 = np.zeros(shape=(rep, s - s1))
for r in range(rep):
    fi3[r] = beta[:, s1:s][r][:, 3]
fi_3 = fi3.reshape((s - s1) * rep, order='F')

fi4 = np.zeros(shape=(rep, s - s1))
for r in range(rep):
    fi4[r] = beta[:, s1:s][r][:, 4]
fi_4 = fi4.reshape((s - s1) * rep, order='F')

print('beta[0] %f' % np.mean(fi_0))
print('beta[1] %f' % np.mean(fi_1))
print('beta[2] %f' % np.mean(fi_2))
print('beta[3] %f' % np.mean(fi_3))
print('beta[4] %f' % np.mean(fi_4))
...
def f(y1, beta1, sigma1):
    ...
    for k in range(n - 4):
        if y1[k + 4] <= (
                beta1[0] + y1[k + 3] * beta1[1] + y1[k + 2] * beta1[2] + y1[k + 1] * beta1[3] + y1[k] * beta1[4]):
            q1[k] = np.exp(
                -(beta1[0] + y1[k + 3] * beta1[1] + beta1[2] * y1[k + 2] + beta1[3] * y1[k + 1] + beta1[4] * y1[k] - y1[
                    k + 4]) * 2 / sigma1) / sigma1
        else:
            q1[k] = np.exp(-(y1[k + 4] - beta1[0] - beta1[1] * y1[k + 3] - beta1[2] * y1[k + 2] - beta1[3] * y1[k + 1] -
                             beta1[4] * y1[k]) * 2 / sigma1) / sigma1
    return np.prod(q1)
...
for q in range(rep):
    ...
    for z in range(1, s):
        ...
        la1 = min(1.0, (f(y[q], ab1, sigma[q][z - 1]) / f(y[q], beta[q][z - 1], sigma[q][z - 1])) *
                  (st.multivariate_normal.pdf(ab1, np.array([0, 0, 0, 0, 0]), np.array([[5, 0, 0, 0, 0],
                                                                                        [0, 5, 0, 0, 0],
                                                                                        [0, 0, 5, 0, 0],
                                                                                        [0, 0, 0, 5, 0],
                                                                                        [0, 0, 0, 0, 5]])) /
                   st.multivariate_normal.pdf(beta[q][z - 1], np.array([0, 0, 0, 0, 0]),
                                              np.array([[5, 0, 0, 0, 0], [0, 5, 0, 0, 0],
                                                        [0, 0, 5, 0, 0], [0, 0, 0, 5, 0],
                                                        [0, 0, 0, 0, 5]]))) *
                  (st.multivariate_normal.pdf(beta[q][z - 1], mean=mu_beta[q][z - 1], cov=sigma_beta[q][z - 1]) /
                   st.multivariate_normal.pdf(ab1, mean=mu_beta[q][z - 1], cov=sigma_beta[q][z - 1])))
    ...
    def g(y1, k1, beta1, sigma1):
        if y1[k1 + 4] <= (
                beta1[0] + y1[k1 + 3] * beta1[1] + y1[k1 + 2] * beta1[2] + y1[k1 + 1] * beta1[3] + y1[k1] * beta1[4]):
            q1 = np.exp(-(beta1[0] + y1[k1 + 3] * beta1[1] + beta1[2] * y1[k1 + 2] + beta1[3] * y1[k1 + 1] + beta1[4] *
                          y1[k1] - y1[k1 + 4]) * 2 / sigma1) / sigma1
        else:
            q1 = np.exp(-(
                    y1[k1 + 4] - beta1[0] - beta1[1] * y1[k1 + 3] - beta1[2] * y1[k1 + 2] - beta1[3] * y1[k1 + 1] -
                    beta1[4] * y1[k1]) * 2 / sigma1) / sigma1
        return q1
