...
for e in range(rep):
    for i in range(2, n):
        ...
        y[e][i] = beta_true[0] + y[e][i - 1] * beta_true[1] + y[e][i - 2] * beta_true[2] + ran_aep(p1_t, p2_t, sigma_t)
...
gamma = np.zeros(shape=(rep, s, 3))
omega = np.zeros(shape=(rep, s, 3))
psi, vpi = float(2), float(5)
...
for q in range(rep):
    beta[q][0] = st.multivariate_normal.rvs(np.array([0, 0, 0]), np.array([[5, 0, 0], [0, 5, 0], [0, 0, 5]]), size=1)
    ...
    sigma_beta[q][0] = np.array([[5, 0, 0], [0, 5, 0], [0, 0, 5]])
    ...
    for z in range(1, s):
        ...
        la1 = min(1.0, (f(y[q], ab1, p1[q][z - 1], p2[q][z - 1], sigma[q][z - 1]) /
                        f(y[q], beta[q][z - 1], p1[q][z - 1], p2[q][z - 1], sigma[q][z - 1])) * (
                          st.multivariate_normal.pdf(ab1, mean=np.array([0, 0, 0]),
                                                     cov=np.array([[omega[q][z - 1][0], 0, 0],
                                                                   [0, omega[q][z - 1][1], 0],
                                                                   [0, 0, omega[q][z - 1][2]]])) /
                          st.multivariate_normal.pdf(beta[q][z - 1], mean=np.array([0, 0, 0]),
                                                     cov=np.array([[omega[q][z - 1][0], 0, 0],
                                                                   [0, omega[q][z - 1][1], 0],
                                                                   [0, 0, omega[q][z - 1][2]]]))) *
